function [gen] = third_order1(mpc,gen,Y1,Y2,PV)
...
tB=1/omegaB;
%% 保留原始值
gen.omega1=gen.omega;
gen.delta1=gen.delta;
gen.Eq11=gen.Eq1;
%% 先做一次前向
gen.omega=gen.omega+dt./(gen.Tj).*(gen.Tm-(abs(gen.Eq1).*abs(i_q)-(Xd1-Xq).*abs(i_d).*abs(i_q)));
gen.delta=gen.delta+dt.*(gen.omega-1)*omegaB;
gen.Eq1=gen.Eq1+dt./(Td).*(gen.Ef-(gen.Eq1+(Xd-Xd1).*abs(i_d)));
